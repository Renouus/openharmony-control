...
import { closeDatabase, getDb, initDatabase, seedDemoData } from "../../src/db/database";
...
describe("secure database initialization", () => {
  ...
  it("treats an empty protected string as residual plaintext", () => {
    const dbPath = path();
    const repositories = createTestEncryptedRepositories();
    const db = initDatabase(dbPath, repositories, { mode: "production" });
    db.prepare("INSERT INTO devices(id,name,type,state_json,updated_at,version) VALUES ('empty','Empty','light','',1,1)").run();
    closeDatabase();
    expect(() => initDatabase(dbPath, repositories, { mode: "production" })).toThrow(/empty/);
  });

  it("atomically seeds once across two database connections", () => {
    const dbPath = path();
    const repositories = createTestEncryptedRepositories();
    initDatabase(dbPath, repositories, { mode: "demo" });
    closeDatabase();
    const first = new Database(dbPath);
    const second = new Database(dbPath);
    expect(seedDemoData(first, new DeviceRegistry(), repositories)).toBeGreaterThan(0);
    expect(seedDemoData(second, new DeviceRegistry(), repositories)).toBe(0);
    expect(first.prepare("SELECT count(*) FROM devices").pluck().get()).toBe(new DeviceRegistry().list().length);
    expect(first.prepare("SELECT count(*) FROM automations").pluck().get()).toBe(1);
    const versions = first.prepare("SELECT version FROM devices UNION ALL SELECT version FROM automations ORDER BY version").pluck().all() as number[];
    expect(versions).toEqual(Array.from({ length: versions.length }, (_, index) => index + 1));
    first.close(); second.close();
  });

  it("closes a failed initialization and allows a subsequent valid initialization", () => {
    const invalidPath = path();
    writeFileSync(invalidPath, "not a sqlite database");
    expect(() => initDatabase(invalidPath, createTestEncryptedRepositories(), { mode: "production" })).toThrow();
    expect(() => getDb()).toThrow(/not initialized/i);
    expect(() => initDatabase(`${invalidPath}.valid`, createTestEncryptedRepositories(), { mode: "production" })).not.toThrow();
  });
});
